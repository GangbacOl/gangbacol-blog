...
import { uploadPostImages, uploadPost } from "../../apis/post";
...
const SubmitPage = () => {
  ...
  const drawFileList = (fileList: FileList | null) => {
    if (fileList !== null)
      return Array.from(fileList).map((file: any, idx) => <UploadedFile key={idx}>{file.name}</UploadedFile>);
  };
  ...
  return (
    <Container>
      <SubmitBox>
        <HookForm onSubmit={handleSubmit((data: SubmitParamsIntf) => onSubmit(data, images, markdown))}>
